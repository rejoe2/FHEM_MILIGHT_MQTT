...
sub Set($$$@) {
  ...
  if($command ne '?') {
    if(defined($hash->{'.sendJson'}) and $hash->{'.sendJson'} ne 0) {
      ...
      $value =~ s/^\s+|\s+$//g;
      #$msgid = send_publish($hash->{IODev}, topic => $hash->{publishSets}->{$command}->{topic}, message => $hash->{'.sendJson'}, qos => $hash->{qos}, retain => $hash->{retain});
...
sub Attr($$$$) {
  ...
  ATTRIBUTE_HANDLER: {
	...
	$attribute eq "receiveJson" and do {
      if ($command eq "set") {
        $hash->{'.receiveJson'} = $value;
      } else {
        if (defined $hash->{'.receiveJson'}) {
          delete $hash->{'.receiveJson'};
        }
      }
      last;
    };
...
sub onmessage($$$) {
  ...
  if (defined (my $reading = $hash->{subscribeReadings}->{$topic}->{name})) {
    ...
    if($do) {
	  ...
	  if(defined($hash->{'.receiveJson'}) and $hash->{'.receiveJson'} eq 1 and is_valid_json($message)) {
